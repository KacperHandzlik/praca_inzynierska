...
class ProjectController(app_manager.RyuApp):
    ...
    def start_flow_recalculation_timer(self):
        """
        Starts a timer that triggers flow recalculation periodically.
        """
        threading.Timer(self.flow_recalculation_interval, self.recalculate_and_set_flows).start()

    def recalculate_and_set_flows(self):
        """
        Recalculates and sets the flows for all known hosts.
        """
        print("Recalculating and setting flows")
        # Example: Recalculate and set flows for each host pair
        for src in self.hosts:
            for dst in self.hosts:
                if src != dst:
                    src_dpid, src_port = self.hosts[src]
                    dst_dpid, dst_port = self.hosts[dst]
                    src_ip = f'10.0.0.{src[-1]}'# Define or retrieve IP addresses as needed
                    dst_ip = f'10.0.0.{dst[-1]}'
                    # Call your existing install_paths function
                    self.install_paths(src_dpid, src_port, dst_dpid, dst_port, src_ip, dst_ip)
        # print('bands')
        # print(self.bandwidths)
        # Restart the timer
        self.start_flow_recalculation_timer()
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        for p in ev.msg.body:
            if switch.id in [1,2,3,4] and p.port_no == 5:
                ...
                self.bandwidths[switch.id][p.port_no] = 50000
            print(switch.id, " ", p.port_no, " " , p.curr_speed)
